...
def scrape_match(url):
    ...
    if goal_row:
        ...
        for cell_idx, cell in enumerate(goal_cells):
            ...
            for link in player_links:
                ...
                print(f"DEBUG: Player: {player_name} -> first: {first_name}, last: {last_name}")
                
                if not last_name:
                    continue
                
                # Get the parent element's full text to find goal markers
                parent_text = cell.get_text()
                print(f"DEBUG: Parent cell text: {parent_text}")
                
                # Find this player's name in the parent text and get what follows
                player_pattern = re.escape(player_name) + r"[^\n]*"
                player_match = re.search(player_pattern, parent_text)
                
                if player_match:
                    player_line = player_match.group(0)
                    print(f"DEBUG: Player line: {player_line}")
                    
                    # Count goal markers (apostrophe followed by digits)
                    goal_minutes = re.findall(r"'(\d+)", player_line)
                    print(f"DEBUG: Goal minutes found: {goal_minutes}")
                    
                    if last_name and goal_minutes:
                        # Add to existing count if player already has goals
                        if last_name in goal_scorers:
                            goal_scorers[last_name] += len(goal_minutes)
                        else:
                            goal_scorers[last_name] = len(goal_minutes)
                        print(f"DEBUG: Added {len(goal_minutes)} goals for {last_name}")
    else:
    ...
    for idx, table in enumerate(team_tables[:2]):
        ...
        for row in rows:
            ...
            goals = 0
            if last_name in goal_scorers:
                goals = goal_scorers[last_name] * 2
